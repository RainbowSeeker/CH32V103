...
//-----------------引脚定义------------------------------
#ifdef SimIIC
#define TFT_SCL_PIN      B11
#define TFT_SDA_PIN      B10
#define TFT_REST_PIN     B2
#define TFT_DC_PIN       B1
#define TFT_CS_PIN       B0
#define TFT_SDA(x)       ((x == 0)? (GPIO_PIN_RESET(TFT_SDA_PIN))  :(GPIO_PIN_SET(TFT_SDA_PIN)))
#define TFT_SCL(x)       ((x == 0)? (GPIO_PIN_RESET(TFT_SCL_PIN))  :(GPIO_PIN_SET(TFT_SCL_PIN)))
#else
#define TFT_SPIN        SPI_1           //定义使用的SPI号
#define TFT_SCL_PIN     SPI1_SCK_PA5    //定义SPI_SCK引脚
#define TFT_SDA_PIN     SPI1_MOSI_PA7   //定义SPI_MOSI引脚
#define TFT_SDA_IN      SPI1_MISO_PA6   //定义SPI_MISO引脚  IPS没有MISO引脚，但是这里任然需要定义，在spi的初始化时需要使用
#define TFT_REST_PIN    A4             //液晶复位引脚定义 由于实际通讯未使用D15因此 这里复用为GPIO
#define TFT_DC_PIN      A6             //液晶命令位引脚定义
#define TFT_CS_PIN      B0             //定义SPI_CS引脚
#endif
...
void lcd_displayimage032_zoom(uint8 *p, uint16 width, uint16 height, uint16 dis_width, uint16 dis_height,uint8 sepval);

...
//    spi_gpio_init(sck_pin, mosi_pin, miso_pin, cs_pin);
    ...
    if(SPI1_SCK_PA5 == sck_pin) gpio_init(A5, GPI, 1, SPEED_50MHZ|IN_FLOAT);
    else if(SPI1_SCK_PB3 == sck_pin) gpio_init(B3, GPI, 1, SPEED_50MHZ|IN_FLOAT);
    else if(SPI2_SCK_PB13 == sck_pin) gpio_init(B13, GPI, 1, SPEED_50MHZ|IN_FLOAT);
    else assert(0);         //无此参数进入断言
    ...
    SPI_I2S_ITConfig((SPI_TypeDef *)(SPIN[spi_n]),SPI_I2S_IT_RXNE, ENABLE);  // 使能接收中断

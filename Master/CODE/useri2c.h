...
//IO方向设置
#define SDA_SET_IN()  {gpio_init(C1,GPI,1,IN_PULLUP);}
#define SDA_SET_OUT() {gpio_init(C1,GPO,1,GPIO_PIN_CONFIG);}

//IO操作函数
#define SCL_OUT_L    gpio_set(B1,0) //SCL=0
#define SCL_OUT_H    gpio_set(B1,1) //SCL=1
#define SDA_OUT_L    gpio_set(C1,0) //SDA=0
#define SDA_OUT_H    gpio_set(C1,1) //SDA=1
#define SDA_READ_VAL gpio_get(C1)  	//输入SDA
/* Exported functions ------------------------------------------------------- */
